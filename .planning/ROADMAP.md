...
- [x] **Phase 2: June 11 MVP — League + Props + Scoring + Leaderboard + Admin + Ship Gate + Scope Expansion** — End-to-end predict-lock-score-reveal-leaderboard flow; private props nested under /me; admin result entry + auto-fetch via Supabase pg_cron; read-only bracket view; Playwright smoke; family invite distributed (completed 2026-05-27)
...
- [x] 02-05-PLAN.md — Wave 4: Admin score entry — `/admin/matches?mode=view|entry` toggle + AdminResultInputs + saveResult Server Action (sweep + UPSERT idempotency)
...
- [x] 02-12-PLAN.md — Wave 9 (scope expansion): Auto-fetch match scores — migration 0012_pg_cron_score_fetch.sql + 0014_fixtures_auto_fetched_at.sql, /api/score-fetch POST route with Bearer auth, football-data.org v4 client, Wave-0 fixture-mapping unit test
...
| 2. June 11 MVP (League + Props + Scoring + Leaderboard + Admin + Ship Gate) | 12/12 | Complete   | 2026-05-27 |
