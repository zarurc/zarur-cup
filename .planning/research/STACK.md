...
**Domain:** Bilingual (Hebrew RTL + English LTR) family-scale prediction-pool web app
**Researched:** 2026-05-23
**Confidence:** HIGH

## Executive Summary

The user-proposed stack — Next.js App Router + Tailwind + Supabase + Vercel — is the correct 2026 default for this exact shape of app, and the research validates it on every dimension. The only meaningful design tension is **auth**: Supabase's invite-code-without-email use case maps cleanly to `signInAnonymously()` only if you accept device-locked sessions (you should — the family is ~15 people, RLS handles cheating, and a "rename your display name" admin escape hatch covers losses). The rest of the stack is uncontroversial: `next-intl` v4 for i18n, Tailwind v4 with logical-property utilities for RTL, `@supabase/ssr` for cookie-based SSR auth, server actions + Zod 4 for mutations, native `Intl.DateTimeFormat` for kickoff times, Playwright-only for tests (skip Vitest on a 3-week timeline). Free-tier limits on Vercel + Supabase are 1-2 orders of magnitude above what 15 users + 64 fixtures will produce. **The only real risk is Supabase free-tier project auto-pause** after 7 days of inactivity — mitigate with a Vercel Cron ping.
...
**Recommendation: Supabase `signInAnonymously()` + an `invite_codes` table + a `profiles` table with `display_name`. Gate access via RLS using `auth.uid()` and the anonymous JWT claim.** Confidence: **HIGH**.
...
Anonymous Supabase sessions are **device-locked**: they live in the browser's localStorage / cookies. If a user clears storage, switches browser, or switches device, they get a *new* anonymous user UUID and lose their predictions. **Mitigations for a 15-person family pool:**

1. **Admin "merge users" escape hatch** — admin UI lets you reassign predictions from `user_id_old` → `user_id_new` if Cousin Dani logs in on his phone after picking on his laptop. Cheap to build, perfect for trust-based pool.
2. **Tell users in onboarding copy** — "Pick the device you'll use for the tournament; sessions are device-local." Hebrew/English microcopy.
3. **Persistent session is on by default** in `@supabase/supabase-js` (`persistSession: true`) — so as long as the user doesn't clear cookies, the session lives indefinitely. For a 1-month tournament that's fine.
...
**Recommendation: Server Components by default + Supabase Realtime for the leaderboard + Server Actions for mutations. Skip TanStack Query entirely.** Confidence: **HIGH**.
...
**Why not TanStack Query?** TanStack docs themselves say: *"If you are just starting out with a new Server Components app, we suggest you start out with any tools for data fetching your framework provides you with and avoid bringing in React Query until you actually need it."* You don't have infinite scrolling, complex optimistic UIs, or background sync needs. 15 users + 64 matches = stateless server-rendered pages. Adding RQ is ~30KB of weight and a mental model for no payoff.

**Realtime scaling note:** Each `INSERT` on a watched table triggers one RLS check per subscribed user. With 15 users that's nothing. Don't subscribe to high-traffic tables — only subscribe to `match_results` (you'll write ~64 rows total across the tournament) and let downstream UI recompute from there. Don't subscribe to `predictions` (would fan out across users on every pick).

## Date/Time Strategy

**Recommendation: Native `Intl.DateTimeFormat` + ISO 8601 strings + UTC in DB. No date library.** Confidence: **HIGH**.
...
If you discover you need date *arithmetic* (e.g. "lock predictions 30 minutes before kickoff"), reach for **date-fns v4** (`date-fns@^4`) — modular, tree-shakes locales, TypeScript-first, ~13KB tree-shaken. Don't reach for Day.js (plugin model is dated) or Luxon (heavier, similar API). Don't use Moment.js (deprecated, project explicitly recommends against new use).

**DB note:** Store kickoff times as `timestamptz` in Postgres. Always send UTC ISO 8601 to the client. Let `Intl.DateTimeFormat` handle the user's TZ on render.

## Forms Strategy

**Recommendation: Server Actions + Zod 4 for everything. Layer React Hook Form *only* on the bracket-prediction screen.** Confidence: **MEDIUM-HIGH**.
...
Share the Zod schema between client and server — `schemas/predictions.ts` exports `predictionSchema`, both `zodResolver(predictionSchema)` in RHF and `predictionSchema.parse(formData)` in the server action use the same source of truth.

## i18n Strategy

**Recommendation: `next-intl@^4` with `[locale]` segment in App Router, JSON message files, RTL via `<html dir>`.** Confidence: **HIGH**.
...
src/
  app/
    [locale]/
      layout.tsx         # sets <html lang dir>, wraps in NextIntlClientProvider
      page.tsx           # landing
      picks/page.tsx
  ...
  messages/
    ...
    he.json
  i18n/
    routing.ts           # defineRouting({locales: ['en','he'], defaultLocale: 'he'})
    request.ts           # getRequestConfig — loads message file by locale
...
Per PROJECT.md, default locale is Hebrew with English fallback — `defaultLocale: 'he'`, browser detection on (next-intl middleware does this), persistent toggle stored in a cookie that next-intl reads on next visit.

**Don't use:** `next-i18next` (still primarily Pages Router, App Router support is recent and clunky), a hand-rolled JSON dictionary (you re-implement plural rules, locale routing, fallback chains, ICU formatting — wasted week).
...
Add `src/app/api/heartbeat/route.ts`:

```ts
import { createClient } from '@/lib/supabase/server';
export async function GET() {
  const supabase = createClient();
  await supabase.from('fixtures').select('id').limit(1);
...
Pings every 3 days at noon UTC — well under the 7-day pause threshold.
...
**Never** import `SUPABASE_SECRET_KEY` from a client component. Lint rule: ban `process.env.SUPABASE_SECRET_KEY` outside `src/lib/supabase/server.ts` and server actions.

## Stack Patterns by Variant

**If you want to test the riskiest thing first (scoring math correctness):**
...
**If the family complains about device-locked sessions:**
...
**If the leaderboard feels sluggish during a match:**
...
---
*Stack research for: bilingual family-scale prediction-pool web app, 3-week deadline, free-tier hosting*
*Researched: 2026-05-23*
